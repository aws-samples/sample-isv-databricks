...
official [AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) toolkit.
The committed `architecture.png`/`.svg` are pre-generated; re-running `build_architecture.py`
requires a local icon cache (`icons_b64.json`, not committed), so edit the SVG directly for
small changes.</sub>
